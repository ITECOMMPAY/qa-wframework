 ...
 * и создан механизм WElements.
 *
 * Наследником WElements является WArray - который описывает простой список веб-элементов.
 *
 ...
 * }
 *
 * WElements так же реализует стандартный интерфейс PageObject'а и может перенаправлять вызовы его методов всем элементам
 * своей коллекции.
 *
...
abstract class WElements extends WObject implements IPageObject, FacadeWebElementsListener
{
    ...
    public function returnSeleniumElements() : FacadeWebElements
    {
        ...
        if ($this->facadeWebElements === null)
        {
            ...
            {
                /**
                 * WElements является специальным механизмом для создания коллекции элементов.
     ...
     *
     * По сути, во время вызова WElements->refresh() происходит вызов FacadeWebElements->refresh(), который
     ...
     * передаёт их в метод WElements->fillFrom(), который оборачивает каждый FacadeWebElement в заданный
